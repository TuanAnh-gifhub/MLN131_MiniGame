...
export function JoinPage() {
  ...
  return (
    <AppShell
      title="Chiếc Nón Kỳ Lạ"
      subtitle="Nhập tên và mã phòng để vào trò chơi."
      ...
      <section className="mx-auto w-full max-w-xl">
        <form onSubmit={onSubmit} className="grid gap-5 rounded-2xl border-2 border-yellow-500/40 bg-red-900/90 p-8 shadow-[0_0_25px_rgba(234,179,8,0.2)]">
          <p className="text-sm font-medium text-yellow-200/70">
            Admin tạo phòng tại{' '}
            <Link to="/admin" className="text-yellow-400 font-bold underline hover:text-yellow-300">
              /admin
            </Link>
          </p>
          <label className="grid gap-2">
            <span className="text-sm font-bold uppercase tracking-wider text-yellow-300">Nickname</span>
            <input
          ...
          <label className="grid gap-2">
            <span className="text-sm font-bold uppercase tracking-wider text-yellow-300">Room Code</span>
            <input
